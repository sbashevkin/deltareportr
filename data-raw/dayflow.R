...
if(Download){
  ...
  download.file("https://data.cnra.ca.gov/dataset/06ee2016-b138-47d7-9e85-f46fae674536/resource/21c377fe-53b8-4bd6-9e1f-2025221be095/download/dayflow-results-1997-2018.csv",
                file.path("data-raw", "data", "Dayflow", "Dayflow1997 2018.csv"), mode="wb")
  #Dayflow 1984-1996
...
dayflow<-read_csv(file.path("data-raw", "data", "Dayflow", "Dayflow1997 2018.csv"), col_types = "ddcdddddddddddddddddddddddddd")%>%
  mutate(Date=parse_date_time(Date, "%d-%b-%y", tz = "America/Los_Angeles"))%>%
  select(Date, OUT, X2)%>%
  bind_rows(read_csv(file.path("data-raw", "data", "Dayflow", "Dayflow1984 1996.csv"), col_types = "cddcddddddddddddddddddddddddd")%>%
              mutate(DATE=parse_date_time(DATE, "%d-%b-%y", tz = "America/Los_Angeles"))%>%
