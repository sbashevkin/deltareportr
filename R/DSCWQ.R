...
DSCWQer<-function(Data,
  ...
  Secchisum<-Data%>%
    ...
    dplyr::summarise(SD=sd(.data$Secchi, na.rm=T), Secchi=mean(.data$Secchi, na.rm=T))%>%
    dplyr::ungroup()%>%
  ...
  Salsum<-Data%>%
    ...
    droplevels()%>%
    dplyr::group_by(.data$Region, .data$Year)%>%
    dplyr::summarise(SD=sd(.data$Salinity, na.rm=T), Salinity=mean(.data$Salinity, na.rm=T))%>%
    dplyr::ungroup()%>%
    dplyr::mutate(missing="na")%>%
    tidyr::complete(Year=Start_year:(End_year), .data$Region, fill=list(missing="n.d."))%>%
    dplyr::mutate(missing=dplyr::na_if(.data$missing, "na"))%>%
  ...
  Chlsum<-Data%>%
    ...
    droplevels()%>%
    dplyr::group_by(.data$Region, .data$Year)%>%
    dplyr::summarise(SD=sd(.data$Chlorophyll, na.rm=T), Chlorophyll=mean(.data$Chlorophyll, na.rm=T))%>%
    dplyr::ungroup()%>%
    dplyr::mutate(missing="na")%>%
    tidyr::complete(Year=Start_year:(End_year), .data$Region, fill=list(missing="n.d."))%>%
    dplyr::mutate(missing=dplyr::na_if(.data$missing, "na"))%>%
  ...
  Microsum<-Data%>%
    ...
    pivot_longer(c(.data$Microcystis1, .data$Microcystis2, .data$Microcystis3, .data$Microcystis4, .data$Microcystis5), names_to = "Severity", values_to = "Frequency")%>%
  ...
  Tempsum<-Data%>%
    ...
    dplyr::summarise(SD=sd(.data$Temperature, na.rm=T), Temperature=mean(.data$Temperature, na.rm=T))%>%
    dplyr::ungroup()%>%
    dplyr::mutate(missing="na")%>%
    tidyr::complete(Year=Start_year:(End_year), .data$Region, fill=list(missing="n.d."))%>%
    dplyr::mutate(missing=dplyr::na_if(.data$missing, "na"),
