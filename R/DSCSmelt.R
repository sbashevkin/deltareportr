...
DSCSmelter<-function(Start_year=2002,
  ...
  EDSM<-deltareportr::smelt_edsm%>%
    ...
    dplyr::mutate(Variance=dplyr::replace_na(.data$Variance, 0))%>%
    ...
    dplyr::ungroup()%>%
    dplyr::mutate(l95=qlnorm(0.025, meanlog=log(.data$Abundance/sqrt(1+.data$Abundance_CV^2)), sdlog=log(1+.data$Abundance_CV^2)),
           u95=qlnorm(0.975, meanlog=log(.data$Abundance/sqrt(1+.data$Abundance_CV^2)), sdlog=log(1+.data$Abundance_CV^2)))%>%
  ...
  p$IEP<-ggplot2::ggplot()+
    ...
    ggplot2::geom_point(data=dplyr::filter(IEP_Indices, Year==End_year), ggplot2::aes(x=.data$Year, y=.data$Index, color=.data$Source), size=3, color="firebrick3")+
  ...
  EDSM_out <- EDSM%>%
    dplyr::mutate(Month=month(.data$MonthYear),
